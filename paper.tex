...
as shown in Fig~\ref{fig:local_poly}. The "true" mean the model is applied on true image instead of noised image,
we can see the detail is keeped finely compared to other methods which may over-smooth even in "true" image.
...
it costy computation time. 

And it can also provide variance information. The second subfig show the value of linear, it show the linear
...
in image processing, see Fig~\ref{fig:edge}. It's resonable since the edge is so called high frequency feature, hence the extremely
...
can be approxinated by Monte Calor simulation:

$$
R(h) \approx \frac{1}{T} \sum_{t=1}^T \frac{1}{n}\sum_{i=1}^n(\hat{r}_n(x_i)-r(x_i))^2
$$

The $T$ is number of simulation, here $T=200$ is used. See Fig~\ref{fig:crossvalidation}. It's obvious that the optimal $h \approx 0.75$.
...
Fortunately, the LOOCV is equivalence to remove the center of the kernel used in fitering. 

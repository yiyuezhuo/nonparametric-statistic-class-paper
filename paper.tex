...
by smoothing, histogram, kernel density regression and bootstrap.
\end{abstract}
...
Firstly divide the interval [0,1] (Julia treat intensity as number in $[0,1]$ exactly) into $k=100$ bins, which 
center is denoted by $X_1,\dots,X_k$. Let $N_j$ be the number of observations in bin $j$. define
...
I use kernel regression with $h=1$ to solve it, the result can shown in Fig~\ref{fig:DS_REG}.
...
they're \textit{significant}? Say we observe some a big distance and small distance, it's not determined whether the difference of first one is 
...
$$
v_{boot} = \frac{1}{B} \sum_{b=1}^B \left( T^*_{n,b} - \frac{1}{B}\sum_{r=1}^B T^*_{n,r} \right)
$$
...
image from given image $x(i,j)$, $D(x_t^*,y_t^*)$ be distance of two re-sampled images. Thus we can define $v_{boot}^I$ as:

$$
v_{boot}^I = \frac{1}{B}\sum_{b=1}^B \left( D(x_b^*,y_b^*) - \frac{1}{B} \sum_{r=1}^B D(x_r^*,y_r^*) \right)
$$

Here $B=100$, dividing the image into $3 \times 3$ patches, the result can be shown in Fig~\ref{fig:square33}. 
$D(x,y)$ is computed for evert neighboring pair of pixels. The number after $\pm$ is $\sqrt{v_{boot}}$. It can be show that
...
a idea curve which is similar to one listed in \cite{1}. Variance estimation using smoothing get very exact result. Density estimation
...
converting problem to regression is used to suppress fake minimum therefore may help to automatically cut point detection. Finally, 
how to incorporate the variance information provided by bootstrap into Hu moment to evaluate their difference is shown.

Though those are poor enough methods compared to modern approaches. The inter-discipline relationship is still interesting. 

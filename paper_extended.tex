...
A interesting point of Hu moment is that the data format required is not a strict image, 
we can firstly convert image into such a \emph{old-statistician} style format :
...
and then we can run bootstrap style re-sampling on it and get something like this:
...
where shape is as same as the above one, but it can not correspond/recover to a image obviously, 
but it can also be used to compute Hu moment. The property provide a idea to introduce bootstrap as shown below.
...
Without $\log$ transform, say these raw values being $\exp(\dots)$ is too close to $0$  and hard to distinguish.
But at the same time, $\log$ transformation is not steady when value is more close to $0$(some ugly \emph{nan}),
 so I remove part of Hu moment.

Though the looking grid of dismembered toad may be fun, why is it useful?
In fact, we can extract a rough segmentation from it or a more fine-grained grid as in Quadtree \cite{finkel1974quad} method.
So it can be a pre-processing step in front of KDE/intensity-based segmentation method told above to achieve a better result.
  ...
  for (t,sigma_filter) in enumerate([0.2,0.5,1.0,2.0])
      ...
      for i=1:particles
          toad_noised = clamp.(convert(Array{Float64}, 
            toad + rand(Normal(0,sigma_noise/255), size(toad))), 0.0, 1.0)
          toad_filtered = imfilter(Gray.(toad_noised), K)
          est_mat[t,i] = sqrt.(sum((toad_filtered - toad_noised).^2)/
            (n-2*nu+nu_tilde))
      end
  end
...
\begin{lstlisting}
toad_noised = clamp.(convert(Array{Float64}, toad + rand(Normal(0,25/255),
  size(toad))), 0.0, 1.0)
